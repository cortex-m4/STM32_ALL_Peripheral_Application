...
void Init_Flash(uint8_t run_once){
		...
		if(run_once){
				...
				FLASH_EraseSector(FLASH_Sector_4,VoltageRange_3);				//擦除扇区4
				FLASH_ProgramWord(WRITE_START_ADDR4,0);									//写入扇区5的初始偏移地址
		}
		address_log=Flash_ReadWord(WRITE_START_ADDR4);						//读出上一次的地址
}
...
void flash_save_data(uint8_t *key_name,uint8_t *val,uint8_t size){
		...
		address_log=Flash_ReadWord(WRITE_START_ADDR4);
		//将之前的数据读出到暂存器里
		for(i=0;i<address_log;i++){
				data_buf[i]=Flash_ReadWord(WRITE_START_ADDR3+i);
		}
		FLASH_Unlock();
		//清空所有标志
		FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_OPERR | FLASH_FLAG_WRPERR |FLASH_FLAG_PGAERR | FLASH_FLAG_PGPERR|FLASH_FLAG_PGSERR);
		...
		FLASH_EraseSector(FLASH_Sector_3,VoltageRange_3);	//擦除扇区
		//将之前的数据存入到flash里
		for(i=0;i<address_log;i++){
				FLASH_ProgramByte(WRITE_START_ADDR3+i,data_buf[i]);
		}
		if(same_flag==0){
			for(i=0;i<SAVE_DATA_SIZE;i++){		//存下了名字
					FLASH_ProgramByte(WRITE_START_ADDR3+address_log,save_name_buf[i]);
					address_log += 1;
			}
			for(i=0;i<SAVE_DATA_SIZE;i++){		//存下了数据
					FLASH_ProgramByte(WRITE_START_ADDR3+address_log,save_data_buf[i]);
					address_log += 1;
			}
		}
		//已经存入了10组数据,就从头开始存
		...
		/**********向扇区4存入扇区5新的地址**********/
		FLASH_EraseSector(FLASH_Sector_4,VoltageRange_3);				//擦除扇区4
		FLASH_ProgramByte(WRITE_START_ADDR4,address_log);
		FLASH_Lock();
}
...
uint8_t flash_get_data(uint8_t *key,uint8_t *val,uint8_t size){
		...
		for(i=0;i<SAVE_DATA_SIZE*2*size;i++){
				data_read_buf[i]=Flash_ReadWord(WRITE_START_ADDR3+i);
		}
		...
		for(i=0;i<size;i++){
				...
				if(data_read_buf[i*16]==save_name_buf[0]){
						...
						if(check_counter==8){		//说明一样	读出数据
								for(k=0;k<SAVE_DATA_SIZE;k++){
										//这里的i表示第几组
										save_data_buf[k]=Flash_ReadWord(WRITE_START_ADDR3+i*16+8+k);
								}
