...
uint8_t write_buf1[SAVE_DATA_SIZE]={1,2,3,4,5,6,7,8};		//写入暂存1
uint8_t write_buf2[SAVE_DATA_SIZE]={8,7,6,5,4,3,2,1};		//写入暂存2
uint8_t data_buf1[SAVE_DATA_SIZE];		//读出暂存1
uint8_t data_buf2[SAVE_DATA_SIZE];		//读出暂存2
...
int main(void){
		...
		LED_GPIO_Config();
		Init_Flash(1);		
		flash_save_data("hello123",write_buf1,GROUP_SIZE);
		delay_ms(100);
		flash_get_data("hello123",data_buf1,GROUP_SIZE);
		delay_ms(100);
		flash_save_data("hello123",write_buf2,GROUP_SIZE);
		delay_ms(100);
		flash_get_data("hello123",data_buf2,GROUP_SIZE);
		delay_ms(100);
